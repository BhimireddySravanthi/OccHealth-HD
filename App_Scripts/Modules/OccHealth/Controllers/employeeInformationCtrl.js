angular.module("app")
        .controller("ohEmployeeInformationCtrl", ["$scope", "$filter", "$location", "$modal", "$log", "sharedservices", "utilityservices", "toastr", "$http", "globalobject", "$window", "$timeout", "$q"
        , function ohEmployeeInformationCtrl($scope, $filter, $location, $modal, $log, sharedservices, utilityservices, toastr, $http, globalobject, $window, $timeout, $q) {
            ...
            $scope.PersonnelTypeList = angular.copy(globalobject.PersonnelTypeList);
            ...
            //$scope.EmployeeTypeName = globalobject.PersonnelTypeName;$scope.screenType
            ...
            globalobject.currentEncounter.screenType = "employeeinfo";

            if ($scope.RedirectFrom == "Appointment") {
                $scope.EmployeeTypeId = globalobject.PersonnelTypeID;
                $scope.EncounterObject.PersonnelTypeID = $scope.EmployeeTypeId;

            }
            else {
                if (globalobject.RedirectFrom == "Addencounter") {
                    globalobject.currentEncounter.screenType = "encounter";
                    $scope.EmployeeTypeId = globalobject.PersonnelTypeID;
                    $scope.EncounterObject.PersonnelTypeID = $scope.EmployeeTypeId;
                    //$scope.RedirectFrom = "Appointment";
                }
                else {
                    $scope.EmployeeTypeId = $scope.EncounterObject.PersonnelTypeID;
                    $scope.RedirectFrom = "Encounter";
                }
            ...
            function initPromises() {
                ...
                promise.then(initLoader, Failure1);
                promise.then(loadDepartmentData, Failure1);
                promise.then(CheckForInUseIds, Failure1);
                promise.then(closeLoader, Failure1);
            ...
            //$scope.Employee.OccEmployeePk = $scope.EncounterObject.EmployeePK;

            $scope.Employee.EmployeeType = $scope.EmployeeTypeId;

            $scope.formAuthentication = true;
            function GetEmployeeType(Employee) {
                return Employee.ID == parseInt($scope.EmployeeTypeId);
            }
            $scope.EmployeeTypeName = $scope.PersonnelTypeList.filter(GetEmployeeType)[0].Text;


//            switch (parseInt($scope.EmployeeTypeId)) {
//                case 1001:
//                    $scope.EmployeeTypeName = ages.filter(GetEmployeeType)[0].Text;
//                    break;
//                case 1002:
...
//            }
            ...
            //                        .each(function () {
            //                            if ($(this).val() == undefined || $(this).val() == null || $(this).val() == '' || $(this).val() == "? object:null ?") {
            //                                $scope.formAuthentication = false;
            //                                counterValid = 1;
            //                            }
            //                            else if (value == "StateId" && $(this).val() == "0") {
            //                                $scope.formAuthentication = false;
            //                                counterValid = 1;
            //                            }
            //                        });
            ...
            $scope.back = function () {
                ...
                if ($scope.frmEmpInfo.$dirty) {
                    ...
                    if ($scope.RedirectFrom == "Appointment" || $scope.RedirectFrom == "Addencounter") {
                        $location.path("/add");
                    }

                    else if (globalobject.RedirectFrom == 'Encounter')
                        $location.path("/encounterdetails");

                    //To check after develpoment
            ...
            $scope.funSave = function () {
                ...
                if ($scope.formAuthentication) {
                    ...
                    if ($scope.allowSubmit) {
                        sharedservices.parseDates($scope.Employee);
                        $scope.Employee.Supervisor = $('#ddlSupervisor option:selected').text();
                        if ($scope.Employee.OccEmployeePk == undefined || $scope.Employee.OccEmployeePk == null) {
                            $scope.Employee.OccEmployeePk = 0;
                        }
                        $scope.Employee.DaysPerWeek = $scope.filterData.DaysIDs.toString();
                        //$scope.Employee.DateOfBirth = $scope.Employee.DateOfBirth == null ? null : $scope.Employee.DateOfBirth.toString();
                        $scope.Employee.EmployeeFullNameWithID = $scope.Employee.FirstName + ' ' + $scope.Employee.LastName + ' [' + $scope.Employee.OccHealthEmployeeId + ']';
                        if ($scope.RedirectFrom == "Appointment" && ($scope.Employee.EmployeeType == 1003) && ($scope.EncounterObject.AppointmentDetailID == undefined || $scope.EncounterObject.AppointmentDetailID == null || $scope.EncounterObject.AppointmentDetailID == 0)) {
                            globalobject.employeesSelectedList = [];
                            globalobject.employeesSelectedList.push($scope.Employee);
                            $location.path("/add");
                        }
                        else if ($scope.RedirectFrom == "Addencounter" && ($scope.Employee.EmployeeType == 1003)) {
                            globalobject.employeesSelectedList = [];
                            globalobject.employeesSelectedList.push($scope.Employee);
                            $location.path("/add");
                        }
                        else if ($scope.RedirectFrom == "Addencounter" && ($scope.EncounterObject.AppointmentDetailID == 0) && $scope.Employee.OccEmployeePk != 0 && ($scope.Employee.EmployeeType == 1001 || $scope.Employee.EmployeeType == 1002 || $scope.Employee.EmployeeType == 1004)) {
                            globalobject.employeesSelectedList = [];
                            globalobject.employeesSelectedList.push($scope.Employee);
                            $location.path("/add");
                        }
                        else if ($scope.RedirectFrom == "Appointment" && $scope.Employee.OccEmployeePk != 0 && ($scope.EncounterObject.AppointmentDetailID == undefined || $scope.EncounterObject.AppointmentDetailID == null || $scope.EncounterObject.AppointmentDetailID == 0)) {
                            if (globalobject.employeesSelectedList.length > 0) {
                                $.each(globalobject.employeesSelectedList, function (index, value) {
                                    if (value.OccEmployeePk == $scope.Employee.OccEmployeePk) {
                                        globalobject.employeesSelectedList.splice(index, 1);
                                        globalobject.employeesSelectedList.push($scope.Employee);
                            ...
                            var configs = {
                                url: "/WebServices/Foundation/EmployeeListService.asmx/SaveOccHealthAddEmployeeDetails",
                                data: { Obj: $scope.Employee, appointmentDetailID: $scope.EncounterObject.AppointmentDetailID }
                            };
                            sharedservices.xhrService(configs)
                        .success(function (data, status, headers, config) {
                            if (data.d.IsOK) {
                                if (data.d.Object.IsEmployeeIdAvailable || data.d.Object.IsEmailIdAvailable) {
                                    if (data.d.Object.IsEmailIdAvailable) {
                                        toastr.error($filter('translate')('msgEmployeealreadyexistswiththisEmailID'));
                                        //Employee already exists with this Email ID. Please enter an unique Email ID
                                    }
                                    else {
                                        toastr.error($filter('translate')('msgEmployeealreadyexistswiththisEmployeeID'));
                                        //This employee ID already exists. Please enter an unique employee ID
                                    }
                                }
                                else {
                                    ...
                                    if (($scope.EncounterObject.AppointmentDetailID != undefined && $scope.EncounterObject.AppointmentDetailID != null && $scope.EncounterObject.AppointmentDetailID != 0)) {
                                        globalobject.employeesSelectedList = [];
                                    }
                                    globalobject.employeesSelectedList.push($scope.Employee);
                                    ...
                                    else if ($scope.RedirectFrom == "Addencounter") {
                                        globalobject.employeesSelectedList = [];
                                        globalobject.employeesSelectedList.push($scope.Employee);
                                        $location.path("/add");
                                    }
                ...
                .success(function (data, status, headers, config) {
                    if (data.d.IsOK) {
                        ...
                        if ($scope.Employee.Shift != undefined && $scope.Employee.Shift != null) {
                            // CheckForInUseIds();
                            $timeout(function () {
                                $("#ddlShift").val($scope.Employee.Shift);
                                // $scope.showLoadingOverlay = false;
                            }, 500);
                        }
            ...
            switch (parseInt($scope.EmployeeTypeId)) {
                case 1001:
                    ...
                    else if ($scope.RedirectFrom == "Encounter" || $scope.RedirectFrom == "Addencounter") {
                        MandatoryFields = utilityservices.GetCustomKeyValue('45', 'EmployeeEncounterMandatoryFields');
                    }
                    break;
                case 1002:
                    if ($scope.RedirectFrom == "Appointment") {
                        ...
                        MandatoryFields = utilityservices.GetCustomKeyValue('45', 'UnsupervisedContractEmployeeEncounterMandatoryFields');
                    }
                    break;
                case 1003:
                    if ($scope.RedirectFrom == "Appointment") {
                        MandatoryFields = utilityservices.GetCustomKeyValue('45', 'VisitorAppointmentMandatoryFields');
                    }
                    else if ($scope.RedirectFrom == "Encounter" || $scope.RedirectFrom == "Addencounter") {
                        MandatoryFields = utilityservices.GetCustomKeyValue('45', 'VisitorEncounterMandatoryFields');
                    }
                    break;
                case 1004:
                    if ($scope.RedirectFrom == "Appointment") {
                        ...
                        MandatoryFields = utilityservices.GetCustomKeyValue('45', 'SupervisedContractEmployeeEncounterMandatoryFields');
                    }
                    break;
                default:
                    if ($scope.RedirectFrom == "Appointment") {
            ...
            //            window.localStorage.setItem('MandatoryFields', MandatoryFields);

            //            var a = window.localStorage.getItem('MandatoryFields');
            //            var b = JSON.parse(a);
